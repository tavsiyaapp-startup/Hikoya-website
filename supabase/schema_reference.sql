...
-- обычная строка, редактируемая/удаляемая из /admin/banner (включая бывший
-- "слайд 1", он просто самая ранняя запись по created_at, ничем структурно
-- не выделена). Если title и body пустые — слайд рендерится как картинка на
-- всю ширину баннера, без текстовой панели. Показываются в порядке
...
create table hero_slides (
  ...
  image_url_mobile text,   -- добавлено в 0039: своя картинка для мобильной версии слайда, необязательно
  cta_label_ru text,
  cta_label_uz text,
  cta_url text,
  created_at timestamptz not null default now()
...
-- [2026-08-26] hero_slides.title_ru/title_uz/body_ru/body_uz стали
--   nullable, слайд 1 перестал быть зашитым в JSX и превратился в обычную
--   строку hero_slides (миграция 0024, insert при пустой таблице —
--   тот же текст и картинка /images/banner-write.jpg, что были раньше).
--   Теперь /admin/banner умеет редактировать (не только add/delete) любой
--   слайд, включая бывший "первый", и удаление блокируется, если это
--   последний оставшийся слайд. Текст в слайде необязателен — если title и
...
-- [2026-09-01] hero_slides.image_url_mobile (миграция 0039) — своя картинка
--   для мобильной версии слайда, необязательная. HeroCarousel.tsx рендерит
--   оба <Image>, переключая видимость по брейкпоинту sm (та же граница
--   мобильный/десктоп, что уже использовалась в остальной вёрстке слайда);
